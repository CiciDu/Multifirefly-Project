...
class MultiFF(Env):
    ...
    def __init__(self, num_obs_ff=2, full_memory=3, action_noise_std=0.005, ffxy_noise_std=3, invisible_distance=400,
                 terminal_vel=0.01, num_ff=200, reward_per_ff=100, time_cost=0, dt=0.25, episode_len=1024, print_ff_capture=True):
        ...
        self.print_ff_capture = print_ff_capture
    ...
    def state_step(self, action): 
        ...
        vnoise = torch.distributions.Normal(0, torch.ones([1, 1])).sample() * self.action_noise_std
        wnoise = torch.distributions.Normal(0, torch.ones([1, 1])).sample() * self.action_noise_std
    ...
    def beliefs(self):
        ...
        self.angle_to_center_all, angle_to_edge_all = calculate_angle_to_ff(self.ffxy, self.agentx, self.agenty, self.agentheading, 
                                                                            self.reward_boundary, ffdistance=self.ff_distance_all)
        self.visible_ff_indices = find_visible_ff(self.time, self.ff_distance_all, angle_to_edge_all, self.invisible_distance, self.invisible_angle, self.ff_flash)
        ...
        ff_uncertainty_all = (self.full_memory - self.ff_memory_all) * self.ffxy_noise_std
        # update the positions of fireflies with uncertainties added; note that uncertainties are cummulative across steps
        ffx_noisy, ffy_noisy, ffxy_noisy = update_noisy_ffxy(self.ffx_noisy, self.ffy_noisy, self.ffx, self.ffy, ff_uncertainty_all, self.visible_ff_indices)
        ...
        if torch.numel(self.ff_in_memory_indices) >= self.num_obs_ff:
            ...
            self.angle_to_center_topk_noisy, angle_to_edge_topk_noisy = calculate_angle_to_ff(self.ffxy_topk_noisy, self.agentx, self.agenty, self.agentheading, 
                                                                                              self.reward_boundary, ffdistance=self.distance_topk_noisy)
            # Concatenate angles, distance, and memory
            self.ff_array = torch.stack((self.angle_to_center_topk_noisy, angle_to_edge_topk_noisy, self.distance_topk_noisy, 
...
class CollectInformation(MultiFF):  
    ...
    Specifically, env.ff_information has 8 columns: 
    ...
    """
    def __init__(self, num_obs_ff=2, full_memory=3, action_noise_std=0.005, ffxy_noise_std=3, invisible_distance=400,
                 terminal_vel=0.01, num_ff=200, reward_per_ff=100, time_cost=0, dt=0.25, episode_len=16000):
        super().__init__(num_obs_ff=num_obs_ff, full_memory=full_memory, action_noise_std=action_noise_std, ffxy_noise_std=ffxy_noise_std, 
                         invisible_distance=invisible_distance, terminal_vel=terminal_vel, num_ff=num_ff, reward_per_ff=reward_per_ff, time_cost=time_cost, 
    ...
    def beliefs(self):
        ...
        if torch.numel(self.visible_ff_indices) >= self.num_obs_ff:
            ...
            self.angle_to_center_topk_noisy, angle_to_edge_topk_noisy = calculate_angle_to_ff(self.ffxy_topk_noisy, self.agentx, self.agenty, self.agentheading, 
                                                                                              self.reward_boundary, ffdistance=self.distance_topk_noisy)
            # Concatenate angles, distance, and memory
            self.ff_array = torch.stack((self.angle_to_center_topk_noisy, angle_to_edge_topk_noisy, self.distance_topk_noisy), dim=0)
...
def calculate_angle_to_ff(ffxy, agentx, agenty, agentheading, reward_boundary, ffdistance = None):
    ...
    angle_to_edge_abs = torch.clamp(angle_adjusted_abs, min=0)
    # restore the signs of the angles
    angle_to_edge = torch.sign(angle_to_center) * angle_to_edge_abs
    return angle_to_center, angle_to_edge
